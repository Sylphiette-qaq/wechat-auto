...
- [ ] 4.3 通过 noVNC 完成首次登录，确认登录状态 volume 写入
- [ ] 4.4 重启容器确认登录状态和日志 volume 保留
- [ ] 4.5 在前台私聊和群聊中发送普通文本，记录 CLI 是否能输出正文、聊天名和发送者
- [ ] 4.6 若正文无法通过 AT-SPI 读取，保存控件树快照和诊断日志，标记 AT-SPI 阻塞，不引入数据库或 OCR
